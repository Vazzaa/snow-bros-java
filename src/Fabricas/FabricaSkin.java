package Fabricas;

import Entidades.Skin;

public abstract class FabricaSkin {

    abstract Skin crearSkinDemonioRojo();
    abstract Skin crearSkinTrollAmarillo();
    abstract Skin crearSkinCalabaza();
    abstract Skin crearSkinFantasma();
    abstract Skin crearSkinMoghera();
    abstract Skin crearSkinKamakichi();
    abstract Skin crearSkinRanaDeFuego();
    abstract Skin crearSkinPowerUpAzul();
    abstract Skin crearSkinPowerUpRojo();
    abstract Skin crearSkinPowerUpVerde();
    abstract Skin crearSkinFruta();
    abstract Skin crearSkinVidaExtra();
    abstract Skin crearSkinSnowBro();
    abstract Skin crearSkinPincho();
    abstract Skin crearSkinEscalera();
    abstract Skin crearSkinPared();
    abstract Skin crearSkinSueloResbaladizo();
    abstract Skin crearSkinPlataforma();
    abstract Skin crearSkinPlatQuebradiza();
    abstract Skin crearSkinPlatMovil();
    abstract Skin crearSkinProyectilBomba();
    abstract Skin crearSkinProyectilFuego();
    abstract Skin crearSkinProyectilNieve();
    abstract Skin crearSkinBolaDeNieve();
    abstract Skin crearSkinParedDestructible();
}
