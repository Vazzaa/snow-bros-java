package Fabricas;

import Entidades.Skin;

public abstract class FabricaSkin {

    public abstract Skin crearSkinDemonioRojo();
    public abstract Skin crearSkinTrollAmarillo();
    public abstract Skin crearSkinCalabaza();
    public abstract Skin crearSkinFantasma();
    public abstract Skin crearSkinMoghera();
    public abstract Skin crearSkinKamakichi();
    public abstract Skin crearSkinPowerUpAzul();
    public abstract Skin crearSkinPowerUpRojo();
    public abstract Skin crearSkinPowerUpVerde();
    public abstract Skin crearSkinFruta();
    public abstract Skin crearSkinVidaExtra();
    public abstract Skin crearSkinSnowBro();
    public abstract Skin crearSkinPincho();
    public abstract Skin crearSkinEscalera();
    public abstract Skin crearSkinPared();
    public abstract Skin crearSkinSueloResbaladizo();
    public abstract Skin crearSkinPlataforma();
    public abstract Skin crearSkinPlatQuebradiza();
    public abstract Skin crearSkinPlatMovil();
    public abstract Skin crearSkinProyectilBomba();
    public abstract Skin crearSkinProyectilFuego();
    public abstract Skin crearSkinProyectilNieve();
    public abstract Skin crearSkinBolaDeNieve();
    public abstract Skin crearSkinParedDestructible();
}
